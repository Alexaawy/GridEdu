...
#----------------------------------------------------------------------------------------------- Added by AYman
    @api.constrains("timetable_ids")
    def _check_teacher_time_conflict(self):
        """Check for teacher time conflict across all timetables."""
        if self.timetable_type == "regular":
            for line in self.timetable_ids:
                # Search for conflicting lines in all timetables
                conflicts = self.env["time.table.line"].search(
                    [
                ...
                if conflicts:
                    raise ValidationError(
                        _(
                            "Teacher %s has a conflict at %s on %s! There's another "
                            "lecture scheduled during this time."
                        )
                        % (line.teacher_id.name, line.start_time, line.week_day)
                    )
#-----------------------------------------------------------------------------------------------
...
class TimeTableLine(models.Model):
    ...
    def check_time_overlap(self):
        for rec in self:
            if rec.table_id.timetable_type == 'regular':  # Only check for regular timetables
                if self.search(
                    [
                        "&",
                        "&",
