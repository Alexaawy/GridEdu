...
class DailyAttendance(models.Model):
    ...
    def onchange_check_faculty_value(self):
        if self.user_id:
            # Access the standard ID from the selected teacher (assuming a field named 'standard_id')
            self.standard_id = self.user_id.standard_id
            print("Selected Academic Class:", self.user_id.standard_id.name)
            # print("Selected Academic Class:", self.user_id.standard_id.id)
        return {}  # No need to return a domain in this approach
    ...
    date = fields.Date(
        "Date",
        help="Current Date",
        default=lambda *a: time.strftime("%Y-%m-%d"),  # need to be ("%Y/%m/%d"), it was ("%m/%d/%Y"),
    )
    standard_id = fields.Many2one(
        "school.standard",
        "Academic Class",
